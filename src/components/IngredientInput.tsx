...
export default function IngredientInput({
  ingredients, onAdd, onRemove, onSearch, loading,
}: Props) {
    ...
    <div id="ingredients">
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1 relative">
          <div className="flex flex-wrap items-center gap-2 px-4 py-3 min-h-[56px] bg-white/95 dark:bg-[#1a1a2e]/95 border-2 border-orange-200 dark:border-gray-700 rounded-2xl focus-within:border-orange-400 transition-colors shadow-sm">
            ...
            <div className="flex-1 flex flex-wrap items-center gap-1.5 min-w-0 overflow-x-auto pb-0.5">
              {ingredients.map((item, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-orange-50 to-red-50 dark:from-orange-900/30 dark:to-red-900/30 text-orange-700 dark:text-orange-300 text-sm font-medium rounded-lg whitespace-nowrap"
                >
                  {item}
              ...
              <input
                ...
                className="flex-1 min-w-[140px] bg-transparent outline-none text-base text-gray-700 dark:text-gray-200 placeholder-gray-400 py-2"
              />
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {suggestions.slice(0, 12).map((item) => (
                  <button
                    ...
                    className="px-3 py-1.5 text-sm bg-orange-50 dark:bg-gray-800 hover:bg-orange-100 dark:hover:bg-orange-900/30 text-gray-600 dark:text-gray-400 rounded-lg transition-colors"
                  >
                    {item}
                  </button>
                ))}
        ...
        <button
          ...
          className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-3.5 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 disabled:from-gray-300 disabled:to-gray-300 dark:disabled:from-gray-700 dark:disabled:to-gray-700 disabled:cursor-not-allowed text-white text-sm sm:text-base font-bold rounded-xl shadow-lg shadow-orange-500/30 hover:shadow-orange-500/50 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:shadow-none disabled:hover:scale-100 flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mt-5">
          ...
          {["Rice, Egg, Onion", "Pasta, Tomato, Garlic", "Chicken, Potato, Curry"].map((ex) => (
            <button
              key={ex}
              onClick={() => {
                ex.split(", ").filter(Boolean).forEach((item) => {
